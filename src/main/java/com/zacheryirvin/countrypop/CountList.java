...
public class CountList {
  private Map<String, Country> clist;
  ...
  public void addc(Country c) {
    this.clist.put(c.getname(), c);
  }

  public boolean removec(String key) {
    if(this.clist.get(key) != null) {
  ...
  public ArrayList<Country> returnlistfirstalpha(CheckCountry c) {
    ArrayList<Country> temp = new ArrayList<>();
    for(String k : clist.keySet()) {
      if(c.test(clist.get(k))) {
        temp.add(clist.get(k));
      }
    }
    return temp;
  ...
  public Country returnMaxPop() {
    int population = 0;
    Country returnCountry = null;
    for(String k : clist.keySet()) {
      if(clist.get(k).getpopulation() > population) {
        population = clist.get(k).getpopulation();
        returnCountry = clist.get(k);
      }
    }
    return returnCountry;
  }

  public ArrayList<Country> returnlistbypop() {
    ArrayList<Country> temp = new ArrayList<>();
    for(String k : clist.keySet()) {
      temp.add(clist.get(k));
    }
    Collections.sort(temp, new Comparator<Country>() {
      @Override
      public int compare(Country c1, Country c2) {
        return ((Integer) c1.getpopulation()).compareTo(c2.getpopulation());
      }
    });
    return (temp);
  }
